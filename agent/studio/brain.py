...
# Khối này CỐ Ý ngắn. Bản trước dài 4200 ký tự sau nhiều lượt chồng thêm, và mỗi câu thêm vào
# đẩy khối SETTING (thứ định nghĩa bối cảnh, nằm ở đầu) xuống thành một phần nhỏ dần của prompt.
# Đã đo trong ngày: thêm chữ tả bối cảnh làm KẾT QUẢ TỆ ĐI. Nên ở đây mỗi luật viết đúng một
# lần, không diễn giải lại — luật nào cũng là luật đã có ảnh chứng minh là cần.
_SHEET_PAGE = (
    ...
    "Nobody teleports between them: a character keeps their place in the space unless the panels "
    ...
    "inside a panel: no turnaround or expression row, and a location reference that is a grid of "
    "angles is ONE place seen several ways — re-frame it, never copy a cell. "
    "WHERE THE WORDS AND THE REFERENCE IMAGES DISAGREE ABOUT HOW SOMETHING LOOKS, THE REFERENCE "
    "WINS: words only say where to point the camera and what happens. For a detail the reference "
    "does not show, use the nearest thing it does show rather than inventing surroundings"
)
...
def location_recap_clause(handle: str) -> str:
    ...
    return (f"Every panel below is set in that same place. Its background is {handle} exactly as "
            ...
            # ĐỪNG thêm chữ tả bối cảnh vào đây nữa. Đã thử: một câu dặn panel hẹp vẫn phải thấy
            # "sạp hàng, mái bạt, mặt tiền và đúng bề rộng đường" — nghe rất hợp lý — làm CẢ SÁU
            # panel trôi khỏi ảnh mẫu, mất sạch màu và hàng hoá, tệ hơn hẳn lúc chưa có nó. Mọi
            # câu tả chỗ đó bằng chữ đều cạnh tranh với chính ảnh tham chiếu, và model theo chữ.
            # Khối này chỉ nên TRỎ về ảnh, không mô tả thay nó.
